...
export class GeminiOAuthProvider {
  ...
  async getAccessToken(): Promise<string> {
    ...
    const client_id = creds.client_id ?? process.env.GEMINI_OAUTH_CLIENT_ID;
    const client_secret = creds.client_secret ?? process.env.GEMINI_OAUTH_CLIENT_SECRET;

    if (!client_id || !client_secret) {
      throw new Error("Gemini token expired. Set GEMINI_OAUTH_CLIENT_ID and GEMINI_OAUTH_CLIENT_SECRET in .env to enable refresh.");
    }

    const res = await fetch(TOKEN_URL, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ grant_type: "refresh_token", refresh_token: creds.refresh_token, client_id, client_secret }),
    });
    ...
    writeFileSync(CREDS_PATH, JSON.stringify({ ...creds, access_token: data.access_token, expiry_date: newExpiry }, null, 2));
