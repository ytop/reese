...
# WORKSPACE_DIR=$HOME/.reese/workspace
```
...
**Logging:** When running in gateway mode, all major agent events (messages received, LLM calls, tool executions, errors) are logged to `$HOME/.reese/workspace/agent.log` and sent to Telegram. Configure `TELEGRAM_LOG_CHAT_ID` to specify which chat receives logs, or it defaults to the first user in `TELEGRAM_ALLOW_FROM`.
...
- Main session: `$HOME/.reese/workspace/sessions/telegram_{chatId}.json`
- Secondary session: `$HOME/.reese/workspace/sessions/telegram_{chatId}_secondary.json`
...
All memory is stored as human-readable files in `$HOME/.reese/workspace/`:
...
```bash
mkdir -p $HOME/.reese/workspace/skills/my-skill
cat > $HOME/.reese/workspace/skills/my-skill/SKILL.md << 'EOF'
---
name: my-skill
...
$HOME/.reese/workspace/  your data
```
