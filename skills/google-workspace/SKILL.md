...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via TypeScript. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Node.js client libraries.
version: 1.0.0
...
metadata:
  reese:
    ...
    homepage: https://github.com/NousResearch/reese-agent
    related_skills: [himalaya]
---

# Google Workspace

Gmail, Calendar, Drive, Contacts, Sheets, and Docs — all through TypeScript scripts in this skill. No external binaries to install.
...
- `scripts/setup.ts` — OAuth2 setup (run once to authorize)
- `scripts/google_api.ts` — API wrapper CLI (agent uses this for all operations)
...
Once they provide the path (it may be named `googleAPI.json`, `client_secret.json`, or similar):

```bash
cp /path/to/googleAPI.json ~/.reese/google_client_secret.json
```

Or let the agent handle copying and then run:
```bash
$GSETUP --client-secret /path/to/googleAPI.json
```
...
**Important practical findings from real usage:**

- The `drive search` command is **very sensitive** to the query format.
- For MIME type filtering (especially Sheets), **always use `--raw-query` with the exact string** `mimeType='application/vnd.google-apps.spreadsheet'`.
- Searching by name (e.g. "tax") works reliably without `--raw-query`.
- The `--order` parameter is **not supported** in the current wrapper.
- To find recently viewed Sheets, the best working patterns are:
  - `$GAPI drive search "spreadsheet name or keyword"` (simple name search)
  - `$GAPI drive search "" --raw-query "mimeType='application/vnd.google-apps.spreadsheet'" --max 10` (all Sheets, sorted by relevance/modified time)

**Working examples:**

```bash
# Search by name (most reliable)
$GAPI drive search "tax calculator" --max 5

# List all Google Sheets (recommended pattern)
$GAPI drive search "" --raw-query "mimeType='application/vnd.google-apps.spreadsheet'" --max 10

# List PDFs as example of raw query
